    ...
    if (!(subdiaglen + 1 <= 0)) {
      ...
      if ((xnorm != 0.0) || (A[iAii].im != 0.0)) {
        ...
        if (fabs(beta1) < 1.0020841800044864E-292) {
          knt = 0;
          i20 = (iSubdiag + subdiaglen) - 1;
          do {
            knt++;
            for (k = iSubdiag; k <= i20; k++) {
              xnorm = A[k - 1].re;
              A_im = A[k - 1].im;
    ...
    if (i < 3) {
      ...
      if (lastv > 0) {
        if (lastc != 0) {
          ...
          iSubdiag = 0;
          i20 = (iAii + 3 * (lastc - 1)) + 4;
          for (knt = iAii + 4; knt <= i20; knt += 3) {
            ix = iAii;
            beta1 = 0.0;
            c_im = 0.0;
            k = (knt + lastv) - 1;
            for (subdiaglen = knt - 1; subdiaglen + 1 <= k; subdiaglen++) {
        ...
        if (!((-tautmp_re == 0.0) && (-tautmp_im == 0.0))) {
          ...
          for (k = 1; k <= lastc; k++) {
            if ((work[knt].re != 0.0) || (work[knt].im != 0.0)) {
              ...
              ix = iAii;
              i20 = lastv + iSubdiag;
              for (subdiaglen = iSubdiag; subdiaglen + 1 <= i20; subdiaglen++) {
