function bh_4wheel_3dof_and_arena_animate_sfun(block)
%==========================================================================
...
hfig = findobj('Tag', 'TAG_FIG_CAR_AND_ARENA_BIRDSEYE');
if(~isempty(hfig))
    close(hfig)
end

% create a specific figure
hfig = figure('Tag', 'TAG_FIG_CAR_AND_ARENA_BIRDSEYE', 'Name', 'FIG_CAR_AND_ARENA_BIRDSEYE');
hax  = axes('Parent', hfig);

% create an ARENA object
arena_OBJ = bh_3dof_4wheel_vehicle_arena_CLS(  );
arena_OBJ = arena_OBJ.plot_arena(hax);
arena_OBJ.set_obstacle_visibility("show")   %"hide"

% set your car's color
dp_veh_ID_num      = block.DialogPrm(2).Data;
dp_veh_initxytheta = block.DialogPrm(3).Data;
...
hax.Tag    = 'TAG_AX_CAR_AND_ARENA_BIRDSEYE'; 
tgt_tag_ax = 'TAG_AX_CAR_AND_ARENA_BIRDSEYE'; 
